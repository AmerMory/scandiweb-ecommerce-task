...
export default function Card({ product, click, selectedCategory, selectedCurrency, amount, key, handleSelectedProductDetails,selectedProductDetails, setSelectedProductDetails, data }) {
    ...
    return (
        <div className={`card  ${product.inStock === false ? 'empty-card' : ''}`} style={selectedCategory !== product.category && selectedCategory !== 'all' ? { "display": "none" } : null} key={product.id} onMouseOver={handleMouseOver(product.id)} onMouseOut={handleMouseOut}>
            {hoverd === product.id && product.inStock === true ?
                ...
                <div className='add-to-card' onClick={click(product, `${product.prices.map(price => selectedCurrency === price.currency.symbol ? amount = price.amount : null)}`, `${product.id + '-' + Math.random()}`, `${null}`)}>
